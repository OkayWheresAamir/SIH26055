...
**The observation gained a second, opt-in layout on 2026-09-14 (D30 resolved as D75).**
...
feature D30's own measurement showed is AoA's real value (still not built). Full account: D75.
...
**"v2" widened again the same day, to 362, on a direct request (D76).** `pulse_count` is a sixth
...
D75, this one was not free: the three "v2" snapshots D75's own retrain had already produced
...
now returns `{183, 362}`. Full account: D76.

**The first result on the widened "v2" observation landed 2026-09-18 (D77, `MEASURED`, single
...
draw all differ at once. Neither row is promoted as the number to carry forward. Full account: D77,
`EVALUATION.md` §5.

**A band-priority reward was built, trained, and measured null this week (D78, 2026-09-18 to
...
scoped or recommended here, a new experiment if picked up again. Full account: D78; mechanism:
`OBSERVATION_SPACE.md` §2.4; numbers: `MODEL_COMPARISON.md`'s Width 398 section;
`scratch/TRAINING_JOURNEY.md` §18.

**The named paths were tried, the same day, and the answer didn't change (D78 follow-up,
...
code not removed.** Full account: D78 (same entry, extended); numbers: `MODEL_COMPARISON.md`'s
Width 398 section; `scratch/TRAINING_JOURNEY.md` §18.

**The agent was made *online*, in three pieces, on 2026-09-19 (D79, D80, D81 — all `BUILT`, none
...
matched pair and the ablation that would make one are pre-registered in D79 and not run.

**D79 — a fourth layout, "v3" (399-wide = "v2p" + `prev_reward` 1).** This is the RL² construction: a
...
436→400, the same class of width change D49/D55/D67/D76 made — `lstm_v3_seed0`/`lstm_v3_seed1` (both
...
measuring nothing — the lesson D78 paid for twice).
...
**D80 — `episode_slots`, so an episode can outrun a recording.** `constants.py` is untouched (D42's
...
**D81 — online fine-tuning (`rfenv/rl/online.py`), a new mode alongside offline training, not instead
of it.** Every rung in `EVALUATION.md` §5, including both of D81's own arms, is still produced by the
...
segment index**, streamed to `segments.jsonl` as it happens. Full accounts: D79, D80, D81;
mechanism: `OBSERVATION_SPACE.md` §2.5.

**A second recurrent-PPO policy exists, opt-in, and its first result favours the baseline (D82,
...
not conclusive (one seed, no mechanism check yet, same caution as D64/D77/D78's first pairs); not
...
23a (D78's own strongest checkpoint, 73.1% beats-recency-both) — **which corrected the small-sample
...
**A third recurrent-PPO policy exists, opt-in, unmeasured (D83, 2026-09-20).** `BandEncoderLstmPolicy`
...
undifferentiated vector the way the baseline and D82 both do. The hard part is the gather, not the
...
same convention D82 set. 21 tests (`test_band_layout.py`, no training stack needed) + 18
...
800k steps), differing only in `--policy`, not started without being asked. Full account: D83.

**D83 was trained and gave a third trap on interception ratio (D84, 2026-09-20).** Twelve of the 36
...
exactly those twelve bands, closely: 23a wastes 1.73% (73.8% beats-recency-both), rung 26a (D83,
