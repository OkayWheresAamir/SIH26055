...
LADDER: tuple[Rung, ...] = (
    ...
    # D79 matched pair: "v3" (treatment -- prev_reward) vs "v2p"
    ...
    # default (all ones) in both -- no confound with D78's own mechanism,
    ...
    # `current_band` (D79's amendment), narrowing "v3" to 400-wide, then
    # `prev_hit` was removed too, the same day, narrowing it again to 399-wide
    # (D79's second amendment) -- the same class of width change D49/D55/D67/
    # D76 made before it. `require_loadable` now refuses this file by name,
    ...
    Rung("lstm_v3_seed0", "24a", "Recurrent PPO (reward_balance, D79 v3 obs, seed 0, 800k)",
            "Ours. Treatment arm, D79: obs_version='v3' -- prev_reward "
            "appended to v2p. Paired against lstm_v2p_ctrl_seed0 (rung 24b), "
            "identical except obs_version. Needs ScanEnv(obs_version='v3'). "
            "DEAD as of D79's amendments (2026-09-20): trained on the 436-wide "
    ...
    Rung("lstm_v2p_ctrl_seed0", "24b", "Recurrent PPO (reward_balance, v2p obs, D79 control, seed 0, 800k)",
    ...
    # D82 matched pair: architecture, not observation. Identical to rung 24b
    ...
    Rung("lstm_v2p_mlpfeature_seed0", "25a", "Recurrent PPO (reward_balance, v2p obs, MlpFeatureLstmPolicy, seed 0, 800k)",
         "Ours. Treatment arm, D82: --policy MlpFeatureLstmPolicy (obs -> 2-layer "
    ...
    # D82 convergence check: the 600k-step checkpoint-freq snapshot of rung 25a,
    ...
    Rung("lstm_v2p_mlpfeature_seed0_s12", "25b", "Recurrent PPO (reward_balance, v2p obs, MlpFeatureLstmPolicy, seed 0, 600k snapshot)",
         ...
         "instead of the final 800k -- see D82's convergence-check note: training "
    ...
    # D83 x D79's second amendment: BandEncoderLstmPolicy on the narrowed "v3"
    ...
    Rung("lstm_v3_bandenc_noprior_seed2", "26a", "Recurrent PPO (reward_balance, D79-narrowed v3 obs, BandEncoderLstmPolicy, no priority, seed 2, 800k)",
         "Ours. D83's BandEncoderLstmPolicy (obs -> shared per-band encoder -> mean "
         "pool -> concat global -> LSTM -> actor/critic) trained on 'v3' after its "
         "second narrowing (prev_reward only, no prev_hit -- D79's amendment), "
    ...
    Rung("lstm_v3_bandenc_noprior_seed0", "26b", "Recurrent PPO (reward_balance, D79-narrowed v3 obs, BandEncoderLstmPolicy, no priority, seed 0, 800k)",
...
def band_at_slot_from_log(log: list[dict]) -> np.ndarray:
    ...
    # Sized from the log itself, not the constant, so a stitched mission (D80)
