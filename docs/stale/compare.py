...
def _report_md(summary: dict[str, dict], meta: dict, fom_meta: dict | None = None) -> str:
    ...
    if fom_meta is not None:
        op = fom_meta["operating_point"]
        out += [
            "## Figures of merit (all seven PS metrics) — see `figures_of_merit.md`",
            "",
            "The problem statement's seven figures of merit are reported in full in "
            "[`figures_of_merit.md`](figures_of_merit.md), written by the same run. "
            "**Five are policy-independent** — P_d, P_fa, sensitivity, % correct "
            "predictions and average intercept-time error — and identical across "
            "schedulers by design (D70, D33, §3); only average intercept rate and "
            "average reward vary by scheduler. Do not read the five constant columns as "
            "a scheduler result.",
            "",
            f"Over the {fom_meta['n_grids']} stare replay grids: P_d = {op['pd']:.4f} "
            f"over their reference-sweep cells ({op['n_occupied']:,} occupied), "
            f"% correct predictions {fom_meta['pct_correct']['accuracy']:.4f} "
            f"(MCC {fom_meta['pct_correct']['mcc']:.4f}), average intercept-time error "
            f"{fom_meta['intercept_time_error']['mean_abs_error_s']:.2f} s (D70).",
            "",
        ]
    return "\n".join(out)
...
# --------------------------------------------------------------------------- #
# Figures of merit -- the seven PS metrics (D70)
# --------------------------------------------------------------------------- #
#
# The problem statement names seven figures of merit. Five are properties of the
# environment/detector and do not move with the policy -- P_d, P_fa, sensitivity
# (receiver-level, §3) plus % correct predictions and average intercept-time error
# (model-level, §2). Only average intercept rate and average reward vary by
# scheduler. D70 records that reading, and that #6/#7 are read at the model level
# rather than invented as scheduler-side quantities.

FOM_FILENAME = "figures_of_merit.md"


def model_level_figures(config_ids: list[str], seed: int = 0) -> dict:
    """The five policy-independent PS figures of merit over these replay grids (D70).

    P_d, P_fa and sensitivity are the receiver's operating point over the
    reference-sweep population of *these* grids (D33); percentage of correct
    predictions is gate 1's per-dwell agreement (D37); average intercept-time error is
    `validate.intercept_time_error` (`EVALUATION.md` §2). Every one is a property of
    the environment/detector, not the scheduler, so it reads the same for every rung --
    the correct outcome (§3, D21), to be stated and not debugged.

    Computed over the **stare replay grids** of `config_ids` only, never the sampled
    scenarios: a sampled scenario has no underlying scan recording to predict against,
    so #6 and #7 are undefined on it, and its P_d would silently change the population.
    The grids are named in the report so P_d is never quoted without them (D33).

    `gate1`/`intercept_time_error` are imported lazily: they pull `h5py`, which the
    normal ladder does not need, mirroring how `figures()` keeps matplotlib local.
    """
    ...
    acc_by_config = np.array([c["accuracy"] for c in g1.checks], dtype=np.float64)
    ite = intercept_time_error(config_ids, seed)

    def _iqr(a: np.ndarray) -> dict:
        # A grid with no occupied reference-sweep cells has no P_d (roc returns NaN);
        # drop those before the spread rather than let one poison the quartiles.
        a = a[~np.isnan(a)]
        if not a.size:
    ...
    return {
        ...
        "pct_correct": {
            ...
            "n_dwells": int(g1.measured["n_configs"] * g1.measured["n_dwells_per_config"]),
            "band0_predicted": g1.measured["band0_predicted"],
            "band0_recorded": g1.measured["band0_recorded"],
            **{f"acc_{k}": v for k, v in _iqr(acc_by_config).items()},
        },
...
def figures_of_merit_md(summary: dict[str, dict], meta: dict, mf: dict) -> str:
    """The seven-column table: one row per scheduler, one column per PS figure (D70)."""
    op = mf["operating_point"]
    pct = mf["pct_correct"]
    ite = mf["intercept_time_error"]
    stamp = meta["written_utc"]

    def _band(mean, p25, p75, fmt="{:.4f}"):
        return (f"{fmt.format(mean)} <br><sub>[{fmt.format(p25)}, "
                f"{fmt.format(p75)}]</sub>")

    # Policy-independent cells -- identical on every row by design (D70, §3).
    pd_cell = _band(mf["pd"]["pooled"], mf["pd"]["p25"], mf["pd"]["p75"])
    pfa_cell = f"{mf['pfa']:.3e}"
    sens_cell = f"{mf['sensitivity_dbm']:.2f}"
    pct_cell = (f"{pct['accuracy']:.4f} <br><sub>MCC {pct['mcc']:.4f}; "
                f"[{pct['acc_p25']:.4f}, {pct['acc_p75']:.4f}]</sub>")
    ite_cell = _band(ite["mean_abs_error_s"], ite["p25_abs_error_s"],
                     ite["p75_abs_error_s"], "{:.2f}")

    def per_scheduler(agg, metric, fmt="{:.4f}"):
        a = agg.get(metric)
        if not a or not a["n"]:
            return "—"
        return (f"{fmt.format(a['mean'])} <br><sub>[{fmt.format(a['p25'])}, "
    ...
    out = [
        ...
        "",
        f"`python -m rfenv.compare --figures-of-merit` — {stamp}. "
        f"{meta['n_scenarios']} scenarios × {len(meta['seeds'])} seed(s) = "
        f"{meta['n_runs']} episodes, train split only, stare replays and sampled "
        f"scenarios only (D36). Reward `{meta['reward']}` throughout.",
        "",
        "The problem statement asks for *\"figures of merit for interception "
        "performance such as probability of detection, probability of false alarm, "
        "sensitivity, average intercept rate, average reward/cost function, percentage "
        "of correct predictions, and average intercept time error.\"* All seven are "
        "below, one column each.",
        "",
        "**Five of the seven are policy-independent** and read the same down their "
        "column by design — that is the correct outcome, not a bug to fix (§3, D21, "
        "D70):",
        "",
        "- **P_d, P_fa, sensitivity** are receiver properties at the frozen operating "
        "point (§3, D25, D33). No scheduler and no reward can move them — penalising a "
        "false alarm prices a wasted dwell, it does not improve the detector.",
        "- **% correct predictions** and **average intercept-time error** are "
        "*model-level* metrics (§2): they measure whether the stare-built environment "
        "reproduces Turing's own sweep — the PS's own text assigns *prediction* to the "
        "system model. They characterise the environment, not any scheduler, so they too "
        "are constant across rungs (D70).",
        "",
        "**Only average intercept rate and average reward vary by scheduler.** Every "
        "mean is printed over its interquartile range because scenario difficulty spans "
        "2 to 99 emitters (EVALUATION.md §7); the two exact analytic constants (P_fa, "
        "sensitivity) have no distribution to show.",
        "",
        "| rung | scheduler | P_d | P_fa | sensitivity (dB) | avg intercept rate (/s) "
        "| avg reward | % correct predictions | avg intercept-time error (s) |",
        "|---|---|---|---|---|---|---|---|---|",
    ]

    for key, block in summary.items():
        agg = block["aggregate"]
        name = f"`{key}`" + (" *(ref. line)*" if block["reference"] else "")
        out.append(
            f"| {block['rung']} | {name} | {pd_cell} | {pfa_cell} | {sens_cell} | "
            f"{per_scheduler(agg, 'avg_intercept_rate_per_s', '{:.3f}')} | "
            f"{per_scheduler(agg, 'total_reward', '{:.2f}')} | "
            f"{pct_cell} | {ite_cell} |"
        )

    out += [
        "",
        "Per-scheduler cells are `mean` over the interquartile range across episodes. "
        "The five constant columns are identical by design; a **reference line** reads "
        "the truth grid and is not a scheduler (§5).",
        "",
        "## Which grids each P_d figure is over (D33)",
        "",
        f"**P_d = {op['pd']:.4f}** here, over the `{op['population']}` cells of *these "
        f"{mf['n_grids']} stare comparison grids* ({op['n_occupied']:,} occupied). "
        "D33 froze the *population rule*, not the set of grids, so this is not "
        "interchangeable with:",
        "",
        "- **0.85058** — the `runs/validation` figure, over the 47 **scan-replay** "
        "grids `validate.py` builds (11,710 occupied). Same rule, different world.",
        "- the P_d over the *full* comparison set when sampled scenarios are included "
        "(D46 measured 0.8421 over 57 stare-and-sampled grids). This report's P_d is "
        "over the replay grids **only**, because #6 and #7 are undefined on a sampled "
        "scenario (no underlying scan recording).",
        "",
        "Quote whichever figure you name the grids for, and never one without them "
        "(D33, §7).",
        "",
        "## Frozen and policy-independent — stated so nobody \"fixes\" it (§3, D70)",
        "",
        "P_d, P_fa and sensitivity are detector properties at the frozen γ and σ; "
        "% correct predictions and average intercept-time error are model-faithfulness "
        "properties of the stare-built environment. **None of the five moves with the "
        "policy.** They read near-identical across `round_robin`, `recency`, `camper` "
        "and every `lstm_*` rung, and that is correct — it must not be debugged. Only "
        "average intercept rate and average reward are scheduler results.",
        "",
        "## Average reward / cost (D7)",
        "",
        f"Scored under reward `{meta['reward']}` — the selected reward (D47/D68). "
        "Average reward is *comparable only within a reward family and never used to "
        "rank across different rewards* (D7); it is a per-family scale, not a "
        "cross-reward score.",
        "",
        "## Average intercept-time error — how it is computed, and its limits (§2, D70)",
        "",
        f"Per emitter, matched by transmitter label: `|predicted − measured|`, where "
        f"*predicted* is the stare-built environment's first-intercept slot under "
        f"Turing's own sweep (D28, censored at 30 s) and *measured* is the emitter's "
        f"first appearance in the actual scan recording (D36). Mean "
        f"**{ite['mean_abs_error_s']:.2f} s** over **{ite['n_emitters_matched']:,}** "
        f"emitters detectable in stare *and* present in scan "
        f"(dropped: {ite['n_predicted_not_in_scan']:,} predicted-only, "
        f"{ite['n_scan_not_predicted']:,} scan-only), seed {ite['seed']}.",
        "",
        "Two limitations, stated rather than patched — the same two gate 1 carries:",
        "",
        "- **scan and stare are independent runs** (D24), so the same emitter has "
        "disjoint activity in each. Part of this error is that realisation divergence, "
        "not a modelling defect — but an out-of-sample prediction (stare → scan) is "
        "exposed to exactly it, so it belongs in the number. This is why the figure is "
        "seconds, not the sub-slot agreement of gate 1's per-dwell view.",
        f"- **band 0 (250 MHz) is invisible to stare** (D10): {pct['band0_recorded']:.1%} "
        f"occupied in the recordings, {pct['band0_predicted']:.1%} predicted, so its "
        "emitters are predicted as never-intercepted and inflate the error where scan "
        "caught them.",
        "",
        "% correct predictions carries the same band-0 limitation, and is reported with "
        "MCC beside it because §7 forbids a bare accuracy on sparse occupancy.",
        "",
        "## Provenance",
        "",
        f"- Written {stamp} (UTC).",
        f"- Scheduler columns: {meta['n_scenarios']} scenarios "
        f"({meta['n_replays']} stare replays, {meta['n_sampled']} sampled), "
        f"seeds {meta['seeds']}, split `{meta['split']}`, reward `{meta['reward']}`.",
        f"- Model-level columns (#6, #7) and P_d: over the {mf['n_grids']} stare replay "
        f"configs, seed {mf['seed']}.",
        f"- Operating point: γ = {op['gamma_dbm']} dB, σ = {op['sigma_db']} dB, "
        f"P_fa = {op['pfa']:.3e}, sensitivity {op['sensitivity_dbm']:.2f} dB, "
        f"population `{op['population']}` ({op['n_occupied']:,} occupied cells).",
        f"- Every scheduler row is scored by `metrics.scheduler_metrics()` from the "
        f"artefacts under `{meta['out_root']}`, never from live environment state.",
        "- Scan replays are refused for the scheduler comparison by "
        "`compare._check_comparison_scenario` (D36); the model-level columns use scan "
        "recordings deliberately, which is gates 1–2's domain, not a comparison scenario.",
        "- P_d, % correct predictions and average intercept-time error re-derive from "
        "`receiver.operating_point`, `validate.gate1` and `validate.intercept_time_error` "
        "— no figure is transcribed.",
        "",
    ]
    return "\n".join(out)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--figures-of-merit", dest="figures_of_merit", action="store_true",
                    help="also write figures_of_merit.md: all seven PS figures of merit (D70)")
    ...
    # The five policy-independent PS figures, over the replay grids only (D70). The
    # sampled scenarios have no scan recording to predict against, so #6/#7 are
    # undefined on them and P_d's population would silently change.
    model_figs = None
    if args.figures_of_merit:
        ...
        print(f"computing the five policy-independent figures of merit over "
              f"{len(replay_configs)} replay grids (D70) ...", flush=True)
    ...
    (out_root / "comparison.md").write_text(
        _report_md(summary, meta, fom_meta=model_figs), encoding="utf-8"
    )
    if model_figs is not None:
