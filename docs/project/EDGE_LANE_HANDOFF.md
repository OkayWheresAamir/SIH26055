...
| **An online agent: in-mission adaptation, unbounded missions, a live view** | three pieces, all built 2026-09-19 and none measured yet. A fourth layout, "v3" (436-wide), feeds the recurrent policy its own last action and what that action returned, so it can adapt inside a mission with no gradient step (D75) -- though 36 of those 38 new columns duplicate blocks that already existed, leaving `prev_reward` as the only genuinely new signal. `episode_slots` lets a mission run for hours, stitched from independent 30 s draws, with `constants.py` and its freeze untouched (D76). `rfenv/rl/online.py` keeps taking gradient steps while it scans, fed the `Y`-based reward a fielded receiver could actually compute rather than the truth-fed one it is scored on (D77). `rfenv/live.py` draws the running episode as a terminal heat strip -- no matplotlib, works over SSH. **Nothing is trained on any of it**; the matched pair and a five-arm ablation with a positive control are pre-registered in D75 | RL lane | built, not measured |
