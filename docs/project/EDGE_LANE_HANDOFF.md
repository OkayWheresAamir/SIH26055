...
| **Extra inputs to the agent's observation** | angle of arrival and pulse width now ship as an opt-in "v2" layout (D71, 2026-09-14) alongside the original ("v1", still default); a third layout, "v2p" (D74, 2026-09-18/19), adds a per-episode band-priority signal plus a matching reward term — an operator-supplied "pay more attention to these bands" input, not learned. Trained, compared and ablated twice: first at a conservative scale (control beat treatment, agent never read the signal), then again at ~4x the incentive with double the network capacity and training budget (treatment jumped to this project's best-ever score, 73.1% beats-recency-both) — but a second ablation found the same thing: the agent still never reads the signal, so the jump is read as training-run variance from the bigger/longer run, not the priority mechanism. Not adopted, not promoted, code not removed | RL lane | measured null (D74), tried stronger, same answer |
| **An online agent: in-mission adaptation, unbounded missions, a live view** | three pieces, all built 2026-09-19/20 and none measured yet. A fourth layout, "v3" (399-wide), feeds the recurrent policy what its last action returned, so it can adapt inside a mission with no gradient step (D75). Originally 436-wide with two more blocks, `prev_action` and `prev_hit`; `prev_action` was removed the next day once confirmed redundant with the pre-existing `current_band` block (an LSTM's hidden state can only carry forward what was in its input, and `current_band` has held the last action since "v1") -- narrowed 436->400 in place, the same class of width change D49/D55/D67/D72 made, so `lstm_v3_seed0`/`lstm_v3_seed1` (both complete 800k-step checkpoints) are now dead; `prev_hit` was removed the same day too, narrowing again to 399, at no further checkpoint cost. `prev_reward` remains the only genuinely new signal, and now the only block "v3" carries beyond "v2p". `episode_slots` lets a mission run for hours, stitched from independent 30 s draws, with `constants.py` and its freeze untouched (D76). `rfenv/rl/online.py` keeps taking gradient steps while it scans, fed the `Y`-based reward a fielded receiver could actually compute rather than the truth-fed one it is scored on (D77) -- a *second*, additional driver: offline training (`rfenv.rl.{ppo,recurrent_ppo,dqn}.train()`, frozen checkpoint) is unaffected and stays the default; `online.py` only ever adapts a checkpoint that path already produced. `rfenv/live.py` draws the running episode as a terminal heat strip -- truth included, six states (where an emitter is, true hit, false alarm, missed detection), no matplotlib, works over SSH; a real accessibility check (ported from the `dataviz` skill's validator, `node` unavailable here) found true-hit-vs-miss measuring 4.1 deuteranopia Delta E on the naive status-palette colours, fixed by giving every state its own glyph. **Nothing is trained on any of it**; the matched pair and a four-arm ablation with a positive control are pre-registered in D75 | RL lane | built, not measured |
| **Extra inputs to the agent's observation** | angle of arrival and pulse width now ship as an opt-in "v2" layout (D75, 2026-09-14) alongside the original ("v1", still default); a third layout, "v2p" (D78, 2026-09-18/19), adds a per-episode band-priority signal plus a matching reward term — an operator-supplied "pay more attention to these bands" input, not learned. Trained, compared and ablated twice: first at a conservative scale (control beat treatment, agent never read the signal), then again at ~4x the incentive with double the network capacity and training budget (treatment jumped to this project's best-ever score, 73.1% beats-recency-both) — but a second ablation found the same thing: the agent still never reads the signal, so the jump is read as training-run variance from the bigger/longer run, not the priority mechanism. Not adopted, not promoted, code not removed | RL lane | measured null (D78), tried stronger, same answer |
