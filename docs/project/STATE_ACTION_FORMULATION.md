...
> so a recurrent policy can adapt inside the episode with no gradient step (D79). **36 of those 38
...
> from independent 30 s draws (D80). `constants.py` is untouched — D42's freeze holds and every
...
> gradient fed `reward_balance_obs` rather than `step()`'s truth-fed return (D81). Per-mission
...
>
> **AMENDED 2026-09-20 — (17)** Asked directly whether offline training is still available after D81
...
> D79 entry, extended a second time.
>
